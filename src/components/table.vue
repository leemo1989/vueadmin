	  ...
	  <el-table :data="items"  v-loading="serverTableLoading" size="mini" @filter-change="filterTag">
		...
		<el-table-column
		  ...
		  width="100"
			>
			<template slot-scope="scope">
				<el-button size="mini" v-if="scope.row.log_state == 0"
                        data-toggle="modal"
                        ...
                        data-target="#zcontrol">查看</el-button>
				<el-button size="mini" type="danger" v-if="scope.row.log_state != 0 && scope.row.log_state != 1" @click="tdown(scope.row)">点击通过</el-button>
			</template>
		</el-table-column>
	  </el-table>
